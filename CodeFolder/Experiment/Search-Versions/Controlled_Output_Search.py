...
Important
If you think user needs some resources , just give normal suggestion and after ask from  the user if they want to search for online resources and if they  add this phrase "You can refer to the following resources" 
...
def detect_search_intent(response: str) -> bool:
    search_triggers = ['You can refer to the following resources']
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if detect_search_intent(response):
        print(f"Performing web search based on model's response for text: '{text}'")
        query = text.split(maxsplit=1)[1].strip() if ' ' in text else ''
        if query:
            search_results = perform_search(query)
            response += f"\n\nHere are the search results for '{query}':\n\n{search_results}"
        else:
            response += "\n\nPlease provide a search query after the keyword."
